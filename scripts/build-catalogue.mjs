 ...
 *    weights, subsets, designers, popularity rank. Sends **no**
 *    Access-Control-Allow-Origin header, so a browser cannot read it live;
 *    that is why this is baked.
 *
 ...
 * (with cdn.jsdelivr.net/gh as a mirror). The CSS API is used for one thing
 * only: the inline *preview*, where a subsetted woff2 is exactly right.
 *
...
const UA = { 'User-Agent': 'openfont-manager-build' }

async function getJson(url) {
  const headers = { ...UA }
  // Unauthenticated GitHub API calls are limited to 60/hour; a token lifts that.
...
/** dirKey -> { lic, dir, files: [...], sizes: {file: bytes} } */
const repo = new Map()
for (const licDir of Object.keys(LICENSES)) {
  ...
  for (const entry of tree.tree) {
    ...
    if (!rec) {
      rec = { lic: licDir, dir, files: [], bytes: 0 }
      repo.set(key, rec)
    }
    rec.files.push(file)
    rec.bytes += entry.size ?? 0
  }
...
const families = list
  .filter((f) => {
    // The metadata feed lists a few families that are not open source (brand
    // fonts served for specific products). They have no repo files anyway,
    // but be explicit: nothing non-open-source goes in this catalogue.
    if (f.isOpenSource === false) {
      skippedClosed++
      return false
    }
    return true
  ...
  .map((f) => {
    ...
    return {
      ...
      // Total bytes of those files, for the cart's size estimate.
      b: rec ? rec.bytes : 0,
      // Script subsets the family covers (`menu` is Google's own preview cut).
      s: (f.subsets ?? []).filter((s) => s !== 'menu'),
      // Variable-font axis tags, empty for a static family.
      a: (f.axes ?? []).map((x) => x.tag),
      d: (f.designers ?? []).slice(0, 4),
      // Popularity rank from Google (1 = most popular). Missing -> last.
      r: Number.isFinite(f.popularity) ? f.popularity : 99999,
      // YYYY-MM-DD the family was added to Google Fonts.
      t: f.dateAdded ?? '',
    }
  })
...
console.log(
  `Wrote ${families.length} families (${matched} with downloadable TTFs, ` +
    `${skippedClosed} non-open-source skipped) to src/data/google-fonts.json`,
)
