 ...
 * file". **That is not true.** Checked against the live CDN:
 *
 ...
 * A subsetted font is worse than nothing: it installs without complaint and
 * then renders blanks outside its subset.
 *
 ...
 * its single `latin` file *is* the whole font. `SINGLE_SUBSET_ONLY` below
 * enforces that rather than assuming it, so if Fontsource ever adds a
 * multi-subset family from a non-Google source it is dropped instead of
 ...
 * *could* be read live — this is baked purely to keep search offline and
 * instant. **Run it after build-catalogue.mjs**: it reads google-fonts.json to
 * work out what to leave out.
 *
...
/** Licences that permit handing the file to someone else. */
 ...
 * `parseFontName` has stripped a trailing style word — Fontsource publishes
 * `Syne Italic` as its own family, which the app's parser reduces to `Syne`.
 */
...
for (const f of candidates) {
  ...
  families.push({
    ...
    l: licence,
    v: f.variable ? 1 : 0,
    // Latest published version, so a download can be pinned rather than
    // trusting `@latest` to mean the same thing twice.
    e: f.version ?? 'latest',
    t: (f.lastModified ?? '').slice(0, 10),
  })
}
