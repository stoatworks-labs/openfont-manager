...
while IFS= read -r -d '' f; do files+=("$f"); done < <(
  find fonts -type f \\( -iname '*.ttf' -o -iname '*.otf' -o -iname '*.ttc' \\) -print0 2>/dev/null | sort -z
)
...
count=0
find fonts -type f \\( -iname '*.ttf' -o -iname '*.otf' -o -iname '*.ttc' \\) 2>/dev/null | sort > .fontlist.$$
...
export function readmeText(title: string, generated: string, families: number, files: number): string {
  return `FONT BUNDLE — ${title}
Generated ${generated} by OpenFont Manager
${families} famil${families === 1 ? 'y' : 'ies'}, ${files} font file${files === 1 ? '' : 's'}

WHAT THIS IS
  Open-source fonts collected from Google Fonts and Fontsource, as the
  original complete font files (not the subsetted web versions), ready to
  install. See MANIFEST.txt for the full list, where each font came from, and
  its licence. Each family's own licence text is alongside its files.
...
HOW TO INSTALL
  ...
  macOS
    ...
    No-script alternative: open the "fonts" folder, select the files,
  ...
  Windows
    ...
    No-script alternative: open a family's folder, select the files,
  ...
  Or use the OpenFont Manager desktop app, which installs straight from the
  catalogue — and, at login, from a folder or network share you point it at.
...
LICENSING
  Everything in this bundle is under an open licence — OFL-1.1, Apache-2.0,
  UFL-1.0, MIT, CC0 or similar — and may be installed and passed on freely.
  Keep the licence text with the fonts if you redistribute them: the OFL in
  particular asks for it. MANIFEST.txt records the licence of every file.
`
}
